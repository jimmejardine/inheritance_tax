...
**Executive summary.** Market capitalism's record — a roughly thirty-fold rise in living standards since 1800, the fastest poverty collapse in human history, an unmatched engine of innovation, and the dispersal of power that makes political liberty durable — is not incidental to this project but its premise. This file states that record sincerely and identifies the property every one of capitalism's mechanisms shares: they reward *doing*, not *being* — which is exactly the test inheritance cannot pass.
...
The record above is not luck. Each of capitalism's core mechanisms solves a problem no rival system has solved:

| Mechanism | What it solves | Canonical statement |
|---|---|---|
...
## Beyond income: what the market order carries with it
...
These goods are not decorations on the growth statistics; they are what the growth statistics are *for*. The project's claim throughout is that dynastic concentration endangers them ([the-self-destruction-thesis.md](the-self-destruction-thesis.md)).
...
## What all of these mechanisms have in common

Look down the table again. Every mechanism capitalism relies on is a *performance test applied to the living*:

- The price system rewards those who respond to what it signals — an act.
- Competition rewards the firm that serves customers better *this year* — an act.
- Profit rewards value created; loss punishes value destroyed — acts and their absence.
- The innovator's sliver of surplus rewards invention — the paradigm act.
- Dispersed power stays dispersed only while positions must be continually re-earned.

Capitalism's mechanisms reward **doing**. None of them rewards **being** — being well-born, being connected, being the residuary legatee. The system contains exactly one large transaction that transfers command over capital on the basis of being rather than doing, exempt from every test in the table: no price, no competition, no profit-and-loss audit, no contribution. That transaction is inheritance, and it is examined in [markets-vs-dynasties.md](markets-vs-dynasties.md).

The claim of this section is therefore not that capitalism has a flaw *in* its mechanisms. It is that inheritance is not one of its mechanisms at all — a feudal remnant running inside a market system, growing as a share of the whole ([../01-problem/great-wealth-transfer.md](../01-problem/great-wealth-transfer.md)), and capable, if untreated, of overwriting the performance tests with a birth test ([the-self-destruction-thesis.md](the-self-destruction-thesis.md)). Everything this file admires is what the proposal defends.

## What this file does not claim

Sincerity cuts both ways, so three non-claims for the record. First, this is not a brief for laissez-faire absolutism: the mechanisms in the table above require maintained infrastructure — courts, competition law, honest money, disclosure rules — and capitalism's history of pro-market intervention is itself part of the argument ([the-self-destruction-thesis.md](the-self-destruction-thesis.md)). Second, admiring the engine is not denying its exhaust: market economies generate externalities, transitional unemployment, and inequality of outcomes, and the first two are managed by instruments outside this project's scope. Third — and centrally — admiring what living participants do in markets implies nothing about what dead participants' capital should command across generations. That distinction, between the market system and the dynastic transmission running inside it, is the entire subject of [markets-vs-dynasties.md](markets-vs-dynasties.md).
