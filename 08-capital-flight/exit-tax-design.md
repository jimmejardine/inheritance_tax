# Exit-Tax Design: Taxing Those Who Do Leave

**Executive summary.** For the minority who do emigrate, existing law already supplies every component the proposal needs: Canada's deemed disposition on departure, Germany's extended limited liability, the US expatriation regime (§877A plus the under-appreciated §2801 tax on *recipients* of gifts from expatriates), and the UK's own post-2025 residence-based inheritance-tax tail. The proposal assembles these proven parts into a deemed-receipt exit charge plus a 10-year trailing liability, and the CJEU/ECHR case law shows precisely how to draft it so it survives free-movement challenge: assess at exit, defer collection to realisation.
...
| Model | Trigger | Mechanism | Tail | Lesson for the proposal |
|---|---|---|---|---|
...
Restating [`../02-proposal/avoidance.md`](../02-proposal/avoidance.md) §4 in design terms:

1. **Deemed-receipt exit charge.** On ceasing residence, an individual is treated as having received, at market value, unrealised accrual that would otherwise leave the charge — the Canadian deemed-disposition mechanic transposed to the receipts axis. Assessment at exit; collection deferred until actual realisation or transfer, with security, per the CJEU-compliant pattern below.
2. **10-year trailing tail.** Former residents remain within charge for 10 years on (a) UK-situs receipts (indefinitely, via the situs limb) and (b) worldwide receipts if formerly long-term resident — the German §2 AStG and UK 2025-regime precedents.
3. **Recipient-axis permanence (the §2801 borrow).** A UK-resident recipient is taxable on receipts from *anyone*, including a long-emigrated donor. The donor's escape never opens a route for resident heirs — this, not the exit charge, does most of the work, because heirs are the party least likely to emigrate ([friction-and-anchors.md](friction-and-anchors.md)).
4. **Dual nexus backstop.** UK-situs assets remain in charge regardless of everyone's residence ([`../02-proposal/avoidance.md`](../02-proposal/avoidance.md) §5).

The four layers are deliberately redundant: defeating the tax requires defeating all of them simultaneously — the compound improbability quantified in [friction-and-anchors.md](friction-and-anchors.md).

## Legal constraints and how to draft within them

**EU free movement (relevant to EU adopters and to UK–EU coordination).** The CJEU line is well settled: *Lasteyrie du Saillant* (C-9/02, 2004) struck down France's exit tax because immediate collection on unrealised gains burdened establishment; *N* (C-470/04, 2006) upheld a Dutch protective assessment with unconditional deferral; *National Grid Indus* (C-371/10, 2011) confirmed exit charges are compatible where the tax is *fixed at exit but collected at realisation*, without disproportionate guarantees [4]. The drafting rule: **crystallise the liability at departure, defer collection, charge modest interest, demand security only where recovery risk is demonstrable.** ATAD Article 5 now *mandates* corporate exit taxes across the EU — the direction of travel is toward exit charges, not away [4].
...
|---|---|---|
| Tail length | 10 years | Matches German AStG and UK 2025 law; long enough that exit must precede death by a planning horizon few will gamble on; short enough to survive proportionality review |
