# Funding: Who Pays, and How Not to Be Owned

**Executive summary.** Early money will almost certainly come from inequality-focused foundations and wealthy individual defectors — which creates the standing irony of billionaire-funded anti-dynasty work, an attack the movement defuses structurally (irrevocable gifts, funder-diversity caps, radical transparency) rather than rhetorically. The long-run goal is a membership-income base, because money that arrives in small recurring amounts is the only money that cannot be withdrawn by a single phone call.

## The funding landscape

Philanthropic funders with current or historical inequality programmes (all programme details to verify before approach):
...
## Transparency-as-armour doctrine

The opposition's cheapest attack is the astroturf allegation — that the movement is a front for hidden interests ([10-opposition/astroturf-and-lobbying.md](../10-opposition/astroturf-and-lobbying.md)). The defence is to be *more* transparent than any attacker, and more transparent than the attackers themselves:
...
| Income type | Speed | Durability | Strings | Signal value |
|---|---|---|---|---|
| Foundation grants | Slow (6–18 month cycles) | Medium; programme fashions shift | Reporting, scope | Neutral |
| Major individual gifts | Fast | Low unless irrevocable | Highest capture risk | Negative if undisclosed, positive if defector-framed |
...
A published refusal policy is part of the armour:

- Anonymous money above the disclosure threshold — anonymity is the astroturf signature.
- Money from parties currently litigating or lobbying against tax enforcement (test case-by-case).
- Foreign-government money in any form — a free gift to [10-opposition/personal-attacks.md](../10-opposition/personal-attacks.md) otherwise.
- Any gift conditioned on editorial outcomes, however mild the condition.

## Funding the flank

Per the two-organisation model ([organisational-forms.md](organisational-forms.md)): the institute and the radical flank must not share funders in any traceable pattern, or the separation collapses under the first investigation. If the same three donors fund both, opponents are right to call it one operation.
